...
def sales_by_employee():
    ...
    if err:
        return err
    recs = fetch_sales_by(
        "KeyEmployee", request.args["key"], request.args["start"], request.args["end"]
    )
    return jsonify(
        {
            "filters": {
                "KeyEmployee": request.args["key"],
...
def sales_by_product():
    ...
    if err:
        return err
    recs = fetch_sales_by(
        "KeyProduct", request.args["key"], request.args["start"], request.args["end"]
    )
    return jsonify(
        {
            "filters": {
                "KeyProduct": request.args["key"],
                "start": request.args["start"],
                "end": request.args["end"],
            },
            "metrics": aggregate_sales(recs),
...
def sales_by_store():
    ...
    if err:
        return err
    recs = fetch_sales_by(
        "KeyStore", request.args["key"], request.args["start"], request.args["end"]
    )
    return jsonify(
        {
            "filters": {
                "KeyStore": request.args["key"],
                "start": request.args["start"],
                "end": request.args["end"],
            },
            "metrics": aggregate_sales(recs),
