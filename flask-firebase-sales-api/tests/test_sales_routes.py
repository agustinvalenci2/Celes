...
class TestSalesRoutes(TestCase):
    ...
    def test_sales_by_employee(self):
        response = self.client.get('/sales/employee/<KeyEmployee>?start_date=<start_date>&end_date=<end_date>')
        self.assertEqual(response.status_code, 200)
        self.assertIn('sales', json.loads(response.data))

    def test_sales_by_product(self):
        response = self.client.get('/sales/product/<KeyProduct>?start_date=<start_date>&end_date=<end_date>')
        self.assertEqual(response.status_code, 200)
        self.assertIn('sales', json.loads(response.data))

    def test_sales_by_store(self):
        response = self.client.get('/sales/store/<KeyStore>?start_date=<start_date>&end_date=<end_date>')
        self.assertEqual(response.status_code, 200)
        self.assertIn('sales', json.loads(response.data))
